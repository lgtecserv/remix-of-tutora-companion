      ...
      try {
        ...
        return (enrolls ?? []).map((e) => {
          const courseLessons = (lessons ?? []).filter((l: any) => {
            const mod = unwrapRelation((l as any).modules);
            return mod?.course_id === e.course_id;
          });
        ...
        console.error("[MyCourses] queryFn error:", err);
        return [];
      }
    },
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-4 md:grid-cols-2 lg:grid-cols-3">
        {(data ?? []).map((row: any) => {
          ...
          return (
            <Link key={row.course_id} to="/app/curso/$slug" params={{ slug: safeSlug(course.slug, row.course_id) }} className="overflow-hidden flex flex-col rounded-2xl border border-border bg-card transition hover:border-primary/40">
