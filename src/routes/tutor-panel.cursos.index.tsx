...
export const Route = createFileRoute("/tutor-panel/cursos/")({ component: AdminCourses });
...
function AdminCourses() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      const { data: courses, error } = await supabase.from("courses").select("*").eq("tutor_id", user.id).order("created_at", { ascending: false });
  ...
  async function create() {
    ...
    const { data: created, error } = await supabase.from("courses").insert({ title: title.trim(), slug: slugify(title), tutor_id: user.id }).select().single();
    if (error) return toast.error(error.message);
    toast.success("Curso criado — complete os detalhes");
    setOpen(false); setTitle("");
    qc.invalidateQueries({ queryKey: ["tutor-courses"] });
    navigate({ to: "/tutor-panel/cursos/$id", params: { id: created.id } });
  }

  async function togglePublish(c: any) {
    const { error } = await supabase.from("courses").update({ is_published: !c.is_published }).eq("id", c.id);
    if (error) toast.error(error.message); else qc.invalidateQueries({ queryKey: ["tutor-courses"] });
  }
  ...
  async function duplicate(c: any) {
    ...
    const { error } = await supabase.from("courses").insert({ ...rest, slug: newSlug, title: c.title + " (cópia)", is_published: false, tutor_id: user.id });
    if (error) toast.error(error.message); else { toast.success("Duplicado"); qc.invalidateQueries({ queryKey: ["tutor-courses"] }); }
  }

  async function remove(c: any) {
    if (!confirm(`Excluir o curso "${c.title}"? Esta ação não pode ser desfeita.`)) return;
    const { error } = await supabase.from("courses").delete().eq("id", c.id);
    if (error) toast.error(error.message); else { toast.success("Curso excluído"); qc.invalidateQueries({ queryKey: ["tutor-courses"] }); }
  }
            ...
            {(data ?? []).map((c: any) => (
              <tr key={c.id} className="border-t border-border">
                ...
                <td className="px-4 py-3"><div className="flex gap-1">
                  <a href={`/tutor-panel/cursos/${c.id}`} className="inline-flex h-9 w-9 items-center justify-center rounded-md hover:bg-accent hover:text-accent-foreground text-sm font-medium" title="Editar">
